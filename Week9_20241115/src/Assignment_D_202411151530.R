...
SFCData <- SFCData %>% mutate (dim_Su = freq*Su/((u_sta^2)*1^(2/3)),
                               ...
                               n_Su   = 0.3*(freq*3/aver_u)^(-2/3),
                               n_Sv   = 0.4*(freq*3/aver_u)^(-2/3),
                               n_Sw   = 0.4*(freq*3/aver_u)^(-2/3),
                               n_Su_emp = 79*(freq*3/aver_u)/ (1+263*(freq*3/aver_u)^5/3),
                               n_Sv_emp = 13*(freq*3/aver_u)/ (1+32*(freq*3/aver_u)^5/3),
                               n_Sw_emp = 3.5*(freq*3/aver_u)/ (1+8.6*(freq*3/aver_u)^5/3)
...
#Su
p1  <-  ggplot()+
  geom_point(data = SFCData,aes(x=n_Su,y=dim_Su),color="red",size =3, alpha =0.5)+
  geom_point(data = SFCData,aes(x=n_Su_emp,y=dim_Su),color="black",alpha =0.5)+
  scale_x_log10()+
  scale_y_log10()+
  ggtitle("Assignment 4 C")+
  labs(x = expression(0.3 * n^"-2/3"), y= expression(frac(f * S[u] * "(" * f * ")", u["*"]^"2" * Phi * epsilon^"2/3")))+
  theme(
...
p2  <-  ggplot()+
  geom_point(data = SFCData,aes(x=n_Sv,y=dim_Sv),color="red", size =3, alpha =0.5)+
  geom_point(data = SFCData,aes(x=n_Sv_emp,y=dim_Su),color="black",alpha =0.5)+
  scale_x_log10()+
  scale_y_log10()+
  ggtitle("Assignment 4 C")+
  labs(x = expression(0.4 * n^"-2/3"), y= expression(frac(f * S[v] * "(" * f * ")", u["*"]^"2" * Phi * epsilon^"2/3")))+
  theme(
    panel.grid.major.x = element_blank(),  # Remove vertical major grid lines
    panel.grid.major.y = element_line(color = rgb(0.5, 0.5, 0.5, alpha = 0.5),linetype = "dashed",linewidth =  0.2),  # Set horizontal major grid lines to grey
    panel.grid.minor = element_blank(),
...
p3  <-  ggplot()+
  geom_point(data = SFCData,aes(x=n_Sw,y=dim_Sw),color="red",size =3, alpha =0.5)+
  geom_point(data = SFCData,aes(x=n_Sw_emp,y=dim_Su),color="black",alpha =0.5)+
  scale_x_log10()+
  scale_y_log10()+
  ggtitle("Assignment 4 C")+
  labs(x = expression(0.4 * n^"-2/3"), y= expression(frac(f * S[w] * "(" * f * ")", u["*"]^"2" * Phi * epsilon^"2/3")))+
  theme(
    panel.grid.major.x = element_blank(),  # Remove vertical major grid lines
    panel.grid.major.y = element_line(color = rgb(0.5, 0.5, 0.5, alpha = 0.5),linetype = "dashed",linewidth =  0.2),  # Set horizontal major grid lines to grey
    panel.grid.minor = element_blank(),
